...
const UpdateCourse = () => {
  ...
  const { userDetails: user, selectedCourse } = useSelector(
    (state) => state.user
  );
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        setLoaction(course.location);
        setEnrollmentStatus(course.enrollmentStatus);
  ...
  const handleSubmit = async (e) => {
    ...
    if (image == null) {
      ...
      try {
        await axios.post(`/updateCourse/${selectedCourse}}`, formData, {
          headers: {
